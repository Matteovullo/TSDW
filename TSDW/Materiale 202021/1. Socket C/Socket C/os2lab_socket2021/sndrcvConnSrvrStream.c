...
/* usa send()/recv() con socket stream, quindi in realta`
 * la comunicazione e` connection-oriented e la semantica e` la stessa di inclnt.c
 */
...
////
	...
	while ( 1 ) {
		...
		printf("\nClient@%s,%u connects on socket %d; sends:\n",
		       inet_ntoa(client_addr.sin_addr),
				 ntohs(client_addr.sin_port), connect_socket);
